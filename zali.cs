...
namespace sportzal1
{
    public partial class zali : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e) //обновить
        {
            ...
            {
                MySqlConnection conn = new MySqlConnection("Server=localhost;Database=sportzal1;User=root;Password=2375425;");
                MySqlCommand cmd = new MySqlCommand("update zali set id_zala ='" + textBox1.Text + "', naimenovanie = '" + textBox2.Text + "' where id_zala =" + dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString() + ";", conn);
                Console.WriteLine("update zali set id_zala ='" + textBox1.Text + "', naimenovanie = '" + textBox2.Text + "' where id_zala =" + dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString() + ";");
                conn.Open();
                cmd.ExecuteReader();
                conn.Close();
                getinfo();
        ...
        private void button4_Click(object sender, EventArgs e) //удалить
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                {
                    string del = "delete from zali where id_zala = " + dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString() + ";";
                    do_Action(del);
                    getinfo();
                }
            }
