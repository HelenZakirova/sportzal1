...
namespace sportzal1
{
    public partial class abonementi : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e) //обновить
        {
            ...
            {
                MySqlConnection conn = new MySqlConnection("Server=localhost;Database=sportzal1;User=root;Password=2375425;");
                MySqlCommand cmd = new MySqlCommand("update abonementi set id_abon ='" + textBox1.Text + "', opisanie = '" + textBox2.Text + "', price = '" + textBox3.Text + "', id_zala = '" + textBox4.Text +"' where id_abon =" + dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString() + ";", conn);
                Console.WriteLine("update abonementi set id_abon ='" + textBox1.Text + "', opisanie = '" + textBox2.Text + "', price = '" + textBox3.Text + "', id_zala = '" + textBox4.Text + "' where id_abon =" + dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString() + ";");
                conn.Open();
                cmd.ExecuteReader();
                conn.Close();
                getinfo();
        ...
        private void button4_Click(object sender, EventArgs e) //удалить
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                {
                    string del = "delete from abonementi where id_abon = " + dataGridView1[0, dataGridView1.CurrentRow.Index].Value.ToString() + ";";
                    do_Action(del);
                    getinfo();
                }
            }
